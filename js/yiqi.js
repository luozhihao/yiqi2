...
(function () {
    ...
    var storeDisplay = function() {
        ...
        if (window.localStorage) {
            localStorage.setItem(str, isFirst); 
        }
        else {
            Cookie.write(str, isFirst);  
        }
    };

    // 检测触发是否显示loading
    var strStoreDate = window.localStorage ? localStorage.getItem(str) : Cookie.read(str);  

    if (strStoreDate !== "1") {  
        document.onreadystatechange = finishLoad;
    } else {
        $('.loading').addClass('hidden');
